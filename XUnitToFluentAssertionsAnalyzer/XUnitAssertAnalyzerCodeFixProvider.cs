...
namespace XUnitToFluentAssertionsAnalyzer
{
    ...
    {
        public override ImmutableArray<string> FixableDiagnosticIds => ImmutableArray.Create("XFA001");
        ...
        private async Task<Document> Rewrite(Document document, SyntaxNode nodeToFix, System.Threading.CancellationToken cancellationToken)
        {
            ...
            var method = (IMethodSymbol)editor.SemanticModel.GetSymbolInfo(originalMethod).Symbol;
            if (method == null)
                return document;
            ...
            else if (methodName is "Equal" or "NotEqual" && method.Parameters.Length == 3 && method.Parameters[0].Type.SpecialType is SpecialType.System_DateTime)
            {
                ...
                result = InvocationExpression(
                    MemberAccessExpression(InvokeShould(originalArguments[1]), newMethodName),
                    ArgumentList(originalMethod.ArgumentList.Arguments[0].Expression, CastExpression(PredefinedType(Token(SyntaxKind.IntKeyword)), MemberAccessExpression(Parenthesize(originalMethod.ArgumentList.Arguments[2].Expression), nameof(TimeSpan.TotalMilliseconds)))));
            }
