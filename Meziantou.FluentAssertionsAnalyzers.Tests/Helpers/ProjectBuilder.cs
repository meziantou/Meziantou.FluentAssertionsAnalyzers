...
public sealed partial class ProjectBuilder
{
    ...
    public IDictionary<string, string> AnalyzerConfiguration { get; private set; }
    ...
    {
        var task = NuGetPackagesCache.GetOrAdd(packageName + '@' + version + ':' + path, key =>
        {
    ...
    public ProjectBuilder AddFluentAssertionsApi() =>
        AddNuGetReference("FluentAssertions", "6.7.0", "lib/net6.0/");
    ...
    public ProjectBuilder WithAnalyzerConfiguration(IDictionary<string, string> configuration)
    {
    ...
    {
        AnalyzerConfiguration ??= new Dictionary<string, string>(StringComparer.Ordinal);
